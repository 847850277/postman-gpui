...
### SQL Database Steps (`kind: sql`)

Flows seamlessly interleave HTTP API requests and direct SQL database queries within the same YAML workflow.

Supported database connection schemes:
- **SQLite**: `sqlite:///path/to/db.sqlite`, `sqlite::memory:`, or `:memory:`
- **MySQL**: `mysql://user:password@host:port/database`

```yaml
  - id: step-query-or-insert
    name: Execute Database Query
    request:
      kind: sql
      connection:
        input: db_url  # or literal: "sqlite:///tmp/demo.db"
      query:
        literal: "INSERT INTO users (name, role) VALUES (?, ?);"
      params:
      - output: { step: fetch_user, name: user_name }
      - literal: "admin"
    checks:
    - kind: status
      equals: 200
...
For queries returning rows (`SELECT`, `SHOW`, `PRAGMA`, `EXPLAIN`, `WITH`), the response JSON contains:
- `$.count`: Total number of returned rows
- `$.rows[...]`: Array of JSON objects representing the fetched rows with column names as keys

For mutation statements (`INSERT`, `UPDATE`, `DELETE`, DDL):
- `$.rows_affected`: Number of affected rows
- `$.last_insert_id`: Auto-incremented primary key generated by the insert
