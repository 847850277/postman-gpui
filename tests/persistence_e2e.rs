...
fn current_session_history_replay_restores_the_complete_request_for_mouse_enter_and_space(
    ...
    workspace.read_with(cx, |workspace, _| {
        ...
        assert_eq!(workspace.active_request().unwrap().url(), request_url);
        assert_eq!(
            workspace.active_request().unwrap().params(),
            &[
...
fn json_request_is_recovered_and_replayed_through_the_rendered_history_action(
    ...
    second_workspace.read_with(cx, |workspace, _| {
        ...
        assert!(workspace.active_request().unwrap().url().is_empty());
        assert!(workspace
            .active_request()
            .unwrap()
            .pre_request_script()
    ...
    click(cx, "history-item-0").unwrap();
    second_workspace.read_with(cx, |workspace, _| {
        assert_eq!(workspace.active_request().unwrap().method(), HttpMethod::POST);
        assert_eq!(
            workspace.active_request().unwrap().url(),
            format!("{}/restart-json?tag=rust", server.url())
        );
        assert_eq!(workspace.active_request().unwrap().authorization_kind(), AuthorizationKind::Bearer);
        assert!(workspace.active_request().unwrap().bearer_token().is_empty());
        assert!(workspace.active_request().unwrap().basic_username().is_empty());
        assert!(workspace.active_request().unwrap().basic_password().is_empty());
        assert_eq!(workspace.active_request().unwrap().body_kind(), BodyKind::Json);
        assert_eq!(
            workspace.active_request().unwrap().request_body(),
            RequestBody::Json(json_body.to_string())
        );
    ...
    assert!(matches!(
        second_workspace.read_with(cx, |workspace, _| workspace.active_request().unwrap().response().clone()),
        ResponseState::Historical { entry_id, .. } if entry_id == original_id
    ));
...
fn raw_and_urlencoded_bodies_recover_and_replay_after_restart(test_cx: &mut TestAppContext) {
    ...
    second_workspace.read_with(cx, |workspace, _| {
        assert_eq!(workspace.active_request().unwrap().body_kind(), BodyKind::Raw);
        assert_eq!(
            workspace.active_request().unwrap().request_body(),
            RequestBody::Raw(raw_body.to_string())
        );
    ...
    click(cx, "history-item-1").unwrap();
    second_workspace.read_with(cx, |workspace, _| {
        assert_eq!(workspace.active_request().unwrap().body_kind(), BodyKind::UrlEncoded);
        assert_eq!(
            workspace.active_request().unwrap().request_body(),
            RequestBody::UrlEncoded(encoded_body.to_string())
        );
...
fn multipart_file_and_editor_intent_recover_and_replay_after_restart(test_cx: &mut TestAppContext) {
    ...
    click(cx, "history-item-0").unwrap();
    second_workspace.read_with(cx, |workspace, _| {
        assert_eq!(workspace.active_request().unwrap().body_kind(), BodyKind::Multipart);
        assert_eq!(workspace.active_request().unwrap().request_body(), expected_body);
        assert_eq!(
...
fn missing_multipart_file_after_restart_uses_the_normal_validation_error(
    ...
    match second_workspace.read_with(cx, |workspace, _| {
...
fn non_2xx_recovers_but_cancel_timeout_and_transport_failures_never_persist(
    ...
    cx.run_until_parked();
    assert!(matches!(
        first_workspace.read_with(cx, |workspace, _| workspace
            .active_request()
            .unwrap()
            .response()
            .clone()),
    ...
    cx.run_until_parked();
    assert!(matches!(
        first_workspace.read_with(cx, |workspace, _| workspace.active_request().unwrap().response().clone()),
...
fn successful_clear_stays_empty_after_restart(test_cx: &mut TestAppContext) {
    ...
    first_workspace.read_with(cx, |workspace, _| {
        ...
        assert_eq!(
            workspace.active_request().unwrap().url(),
            "https://unsent.example/keep-editor"
        );
        assert_eq!(
            workspace.history_storage_status(),
            &HistoryStorageStatus::Ready { skipped_rows: 0 }
    ...
    assert!(second_workspace.read_with(cx, |workspace, _| workspace
        .active_request()
        .unwrap()
        .url()
        .is_empty()));
...
fn legacy_schema_migrates_and_a_malformed_newer_row_is_skipped_after_restart(
    ...
    first_workspace.read_with(cx, |workspace, _| {
        ...
        assert_eq!(
            workspace.active_request().unwrap().headers(),
